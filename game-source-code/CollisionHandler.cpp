    ...
    if(!_turret->GetBullets().empty())
	{
	    for(unsigned int i = 0; i < _turret->GetBullets().size(); i++)
		{
		    ...
		    for(unsigned int j = 0; j < _centipede->GetCentipede().size(); j++)
			{
			    ...
			    else if(_turret->GetBullets().at(i).GetSubRegion() !=
			            _centipede->GetCentipede().at(j).GetSubRegion())
				{
				    continue;
				}
			    else if(CheckDistanceBetweenEntities(_centipede->GetCentipede().at(j),
			                                         _turret->GetBullets().at(i)) <
			            (CENTIPEDE_SEGMENT_HIT_RADIUS + BULLET_HIT_RADIUS))
				{
				    ...
				    auto mushroom =
				        Mushroom{_data, _centipede->GetCentipede().at(j).GetTopLeftXPosition(),
...
void CollisionHandler::CheckSegmentMushroomCollisions()
{
    for(unsigned int i = 0; i < _centipede->GetCentipede().size(); i++)
	{
	    for(unsigned int j = 0; j < _field->GetMushrooms().size(); j++)
		{
			...
			{
			    if(CheckDistanceBetweenEntities(_centipede->GetCentipede().at(i),
			                                    _field->GetMushrooms().at(j)) <
			       (CENTIPEDE_SEGMENT_HIT_RADIUS + MUSHROOM_HIT_RADIUS))
				{
				    _centipede->GetCentipede().at(i).SetPoisoned(true);
				    _centipede->GetCentipede().at(i).SetFirstSegment(true);
...
void CollisionHandler::CheckMushroomScorpionCollisions()
{
    for(unsigned int i = 0; i < _field->GetMushrooms().size(); i++)
	{
	    if(!_field->GetScorpions().empty())
		{
			...
			{
			    if(CheckDistanceBetweenEntities(_field->GetMushrooms().at(i),
			                                    _field->GetScorpions().at(j)) <
    ...
    if(!_turret->GetBullets().empty())
	{
	    for(unsigned int i = 0; i < _turret->GetBullets().size(); i++)
		{
		    if(!_field->GetSpiders().empty())
			{
			    ...
			    for(unsigned int j = 0; j < _field->GetSpiders().size(); j++)
				{
				    if(_turret->GetBullets().at(i).GetRegion() !=
				       _field->GetSpiders().at(j).GetRegion())
					{
					    continue;
					}
				    else if(_turret->GetBullets().at(i).GetSubRegion() !=
				            _field->GetSpiders().at(j).GetSubRegion())
					{
					    continue;
					}
				    else if(CheckDistanceBetweenEntities(_field->GetSpiders().at(j),
				                                         _turret->GetBullets().at(i)) <
    ...
    if(!_turret->GetBullets().empty())
	{
	    for(unsigned int i = 0; i < _turret->GetBullets().size(); i++)
		{
		    if(!_field->GetMushrooms().empty())
			{
			    ...
			    for(unsigned int j = 0; j < _field->GetMushrooms().size(); j++)
				{
				    if(CheckDistanceBetweenEntities(_field->GetMushrooms().at(j),
				                                    _turret->GetBullets().at(i)) <
